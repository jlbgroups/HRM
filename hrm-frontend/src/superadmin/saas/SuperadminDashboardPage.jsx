        ...
        <div className="sa-main" style={{ padding: "28px 28px 40px", flex: 1 }}>
          <div style={{ marginBottom: "24px", animation: "fadeUp 0.4s ease both 0.05s" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
              ...
              <span style={{ fontSize: "0.78rem", fontWeight: "600", color: "#DC2626", textTransform: "uppercase", letterSpacing: "0.8px" }}>Superadmin</span>
            </div>
            <h1 className="sa-page-title" style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(1.5rem,4vw,1.85rem)", fontWeight: "700", color: "#111827", margin: 0, lineHeight: 1.2 }}>
              Control Panel
            </h1>
          ...
          <div className="sa-stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: "16px", marginBottom: "24px" }}>
            {stats.map((stat, idx) => (
              <div key={idx} className="stat-card" style={{ backgroundColor: "#fff", borderRadius: "14px", padding: "20px", border: "1px solid #F1F3F9", boxShadow: "0 2px 8px rgba(15,23,42,0.05)", animation: `fadeUp 0.4s ease both ${0.1 + idx * 0.07}s` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "14px" }}>
                  ...
                  <button style={{ background: "none", border: "none", cursor: "pointer", color: "#D1D5DB", padding: "2px" }}><MoreHorizontal size={16} /></button>
                </div>
                <div style={{ marginBottom: "10px" }}>
                  ...
                  <div className="sa-stat-val" style={{ fontSize: "2rem", fontWeight: "700", color: "#111827", lineHeight: 1, fontFamily: "'Playfair Display', serif" }}>
                    {loading ? <span style={{ display: "inline-block", width: "50px", height: "28px", background: "#F3F4F6", borderRadius: "6px" }} /> : (stat.count || 0)}
                  </div>
                </div>
          ...
          <div style={{ backgroundColor: "#fff", borderRadius: "14px", border: "1px solid #F1F3F9", boxShadow: "0 2px 8px rgba(15,23,42,0.05)", overflow: "hidden", animation: "fadeUp 0.4s ease both 0.38s" }}>
            <div className="sa-head-row" style={{ padding: "18px 22px", borderBottom: "1px solid #F1F3F9", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" }}>
              <div>
                <h2 style={{ fontSize: "1rem", fontWeight: "600", color: "#111827", margin: "0 0 2px" }}>Company Management</h2>
                <p style={{ fontSize: "0.78rem", color: "#9CA3AF", margin: 0 }}>{filtered.length} {filtered.length === 1 ? "company" : "companies"} registered</p>
              </div>
              <div className="sa-search-inner sa-table-search" style={{ position: "relative" }}>
                ...
                <input
                  ...
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  style={{ padding: "8px 12px 8px 32px", border: "1.5px solid #E5E7EB", borderRadius: "9px", fontSize: "0.82rem", color: "#374151", backgroundColor: "#F9FAFB", width: "220px", transition: "border-color 0.18s,box-shadow 0.18s" }}
                />
              </div>
            </div>
            <div style={{ padding: "12px 16px 0", display: "none" }} className="sa-mobile-search">
              <div style={{ position: "relative", marginBottom: "4px" }}>
                <Search size={14} style={{ position: "absolute", left: "10px", top: "50%", transform: "translateY(-50%)", color: "#9CA3AF" }} />
                <input className="search-input" placeholder="Search companies..." value={search} onChange={(e) => setSearch(e.target.value)}
                  style={{ width: "100%", padding: "9px 12px 9px 32px", border: "1.5px solid #E5E7EB", borderRadius: "9px", fontSize: "0.875rem", color: "#374151", backgroundColor: "#F9FAFB" }} />
              </div>
            </div>
                ...
                <thead>
                  <tr style={{ backgroundColor: "#FAFBFF" }}>
                    {["#", "Company", "Subscribed On", "Plan", "Price", "Status"].map((h, i) => (
                      <th key={i} className={i === 2 || i === 4 ? "sa-tbl-hide" : ""} style={{ padding: "11px 18px", textAlign: "left", fontSize: "0.72rem", fontWeight: "600", color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.5px", borderBottom: "1px solid #F1F3F9", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan="6" style={{ padding: "40px", textAlign: "center", color: "#9CA3AF", fontSize: "0.875rem" }}>No companies found</td></tr>
                  ) : (
                    filtered.map((company, i) => (
                      <tr key={company._id} className="company-row" style={{ borderBottom: "1px solid #F9FAFB" }}>
                        <td style={{ padding: "13px 18px", fontSize: "0.82rem", color: "#9CA3AF", fontWeight: "500" }}>{String(i + 1).padStart(2, "0")}</td>
                        <td style={{ padding: "13px 18px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                            <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: `hsl(${(company.company_name?.charCodeAt(0) || 65) * 5 % 360},55%,55%)`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "0.75rem", fontWeight: "600", flexShrink: 0 }}>
                              {(company.company_name || "?").slice(0, 2).toUpperCase()}
                            </div>
                            <span style={{ fontSize: "0.875rem", fontWeight: "500", color: "#111827" }}>{company.company_name}</span>
                          </div>
                        </td>
                        <td className="sa-tbl-hide" style={{ padding: "13px 18px", fontSize: "0.855rem", color: "#6B7280" }}>
                          {new Date(company.createdAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}
                        </td>
                        <td style={{ padding: "13px 18px" }}>
                          <span style={{ display: "inline-flex", alignItems: "center", padding: "3px 10px", borderRadius: "20px", fontSize: "0.72rem", fontWeight: "600", backgroundColor: "#EEF2FF", color: "#4F46E5" }}>
                        ...
                        <td style={{ padding: "13px 18px" }}>
                          <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", padding: "3px 10px", borderRadius: "20px", fontSize: "0.72rem", fontWeight: "600", backgroundColor: company.is_active ? "#ECFDF5" : "#FEF2F2", color: company.is_active ? "#059669" : "#DC2626" }}>
                            <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: company.is_active ? "#059669" : "#DC2626" }} />
            ...
            {!loading && filtered.length > 0 && (
              <div style={{ padding: "12px 22px", borderTop: "1px solid #F1F3F9", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px" }}>
                <span style={{ fontSize: "0.78rem", color: "#9CA3AF" }}>Showing {filtered.length} of {companies.length} companies</span>
                <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
