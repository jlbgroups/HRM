...
export default function TransactionsPage() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#F9FAFB", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .filter-btn { border:1.5px solid #E5E7EB; background:#fff; padding:7px 14px; border-radius:8px; font-size:0.82rem; font-weight:500; cursor:pointer; color:#6B7280; transition:all 0.15s; font-family:inherit; }
  ...
  gap: 16px;
      ...
      <div className="tx-main" style={{ marginLeft: `${sidebarWidth}px`, flex: 1, transition: "margin-left 0.25s cubic-bezier(0.4,0,0.2,1)", padding: "28px 28px 40px", minWidth: 0 }}>
        ...
        {toast && (
          <div style={{ position: "fixed", top: 20, right: 20, zIndex: 9999, background: toast.type === "error" ? "#EF4444" : "#10B981", color: "#fff", padding: "12px 20px", borderRadius: 10, fontWeight: 500, boxShadow: "0 4px 16px rgba(0,0,0,0.15)", display: "flex", alignItems: "center", gap: 8, fontSize: "0.875rem", animation: "fadeUp 0.3s ease", maxWidth: "calc(100vw - 40px)" }}>
        ...
        <div style={{ marginBottom: 24, animation: "fadeUp 0.4s ease both 0.05s" }}>
          ...
          <h1 className="tx-page-title" style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(1.5rem,4vw,1.85rem)", fontWeight: 700, color: "#111827", margin: 0, lineHeight: 1.2 }}>
            Transactions
          </h1>
          <p style={{ color: "#9CA3AF", fontSize: "0.85rem", margin: "5px 0 0" }}>Monitor all company subscription payments</p>
        </div>
{stats && (
  <div
    className="tx-stats-grid"
    style={{
      display: "grid",
    ...
    {statCards.map((card, idx) => (
      ...
      >
        ...
        <div className="tx-toolbar" style={{ display: "flex", gap: 10, marginBottom: 18, flexWrap: "wrap", alignItems: "center", animation: "fadeUp 0.4s ease both 0.32s" }}>
          ...
          <span className="tx-count-badge" style={{ marginLeft: "auto", fontSize: "0.78rem", color: "#9CA3AF", background: "#F3F4F6", padding: "5px 12px", borderRadius: 20, whiteSpace: "nowrap" }}>
            {filtered.length} results
          </span>
          <button
            className="tx-add-btn"
            onClick={openAdd}
            style={{ display: "flex", alignItems: "center", gap: 6, background: "#4F46E5", color: "#fff", border: "none", padding: "9px 18px", borderRadius: 10, fontSize: "0.875rem", fontWeight: 600, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 2px 8px rgba(79,70,229,0.25)", whiteSpace: "nowrap" }}
          >
                ...
                <thead>
                  <tr style={{ backgroundColor: "#FAFBFF" }}>
                    {["#ID", "Company", "Amount", "Payment Date", "Status", "Actions"].map((h, i) => (
                      <th key={i} className={i === 3 ? "tx-tbl-hide" : ""} style={{ padding: "11px 18px", textAlign: i === 5 ? "right" : "left", fontSize: "0.72rem", fontWeight: 600, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.5px", borderBottom: "1px solid #F1F3F9", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((tx) => {
                    ...
                    return (
                      <tr key={txId} className="tx-row" style={{ borderBottom: "1px solid #F9FAFB" }}>
                        <td style={{ padding: "13px 18px", fontSize: "0.78rem", color: "#9CA3AF", fontWeight: 500 }}>#{String(txId).slice(-6)}</td>
                        <td style={{ padding: "13px 18px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#EEF2FF", color: "#4F46E5", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "0.75rem", flexShrink: 0 }}>
                              {compName.charAt(0).toUpperCase()}
                            </div>
                            <span style={{ fontSize: "0.875rem", fontWeight: 500, color: "#111827" }}>{compName}</span>
                          </div>
                        </td>
                        <td style={{ padding: "13px 18px", fontWeight: 700, fontSize: "0.95rem", color: "#111827" }}>
                          ₹{parseFloat(tx.amount || 0).toLocaleString()}
                        </td>
                        <td className="tx-tbl-hide" style={{ padding: "13px 18px", fontSize: "0.855rem", color: "#6B7280" }}>
                        ...
                        <td style={{ padding: "13px 18px", textAlign: "right", whiteSpace: "nowrap" }}>
                          <button onClick={() => openEdit(tx)} style={{ background: "#EEF2FF", color: "#4F46E5", border: "none", padding: "6px 10px", borderRadius: 8, fontSize: "0.78rem", fontWeight: 600, cursor: "pointer", marginRight: 6, display: "inline-flex", alignItems: "center", gap: 4 }}>
                            <Edit2 size={12} /> Edit
                          </button>
                          <button onClick={() => setDeleteConfirm(tx)} style={{ background: "#FEF2F2", color: "#EF4444", border: "none", padding: "6px 10px", borderRadius: 8, fontSize: "0.78rem", fontWeight: 600, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4 }}>
                            <Trash2 size={12} /> Del
                          </button>
                        </td>
                      </tr>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "16px" }}>
          <div className="modal-inner" style={{ background: "#fff", borderRadius: 16, padding: 32, width: 460, maxWidth: "100%", boxShadow: "0 24px 64px rgba(15,23,42,0.20)", animation: "fadeUp 0.25s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              ...
              <button onClick={closeModal} style={{ background: "none", border: "none", cursor: "pointer", color: "#9CA3AF", padding: 4 }}><X size={20} /></button>
            </div>
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
                <button type="button" onClick={closeModal} style={{ padding: "9px 20px", border: "1.5px solid #E5E7EB", background: "#fff", borderRadius: 10, fontSize: "0.875rem", fontWeight: 600, cursor: "pointer", color: "#6B7280", fontFamily: "inherit" }}>Cancel</button>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "16px" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 32, width: 420, maxWidth: "100%", boxShadow: "0 24px 64px rgba(15,23,42,0.20)", animation: "fadeUp 0.25s ease" }}>
            ...
            <p style={{ color: "#6B7280", fontSize: "0.875rem", margin: "0 0 24px", lineHeight: 1.6 }}>
              Are you sure you want to delete the transaction for <strong>{deleteConfirm.company_name || deleteConfirm.company_id?.company_name}</strong>? This cannot be undone.
            </p>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
