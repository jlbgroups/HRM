...
function EmployeeSelect({ onSelect, selectedEmployee, onClear, token }) {
  ...
  if (selectedEmployee) {
    return (
      <div style={{ display:"flex", alignItems:"center", gap:"8px", padding:"8px 12px", background:"#EEF2FF", border:"1.5px solid #C7D2FE", borderRadius:"10px", color:"#4F46E5", fontSize:"0.8rem", fontWeight:"500" }}>
...
export default function LetterPage() {
  ...
  const [form, setForm] = useState({ employeeId:"", employeeName:"", employeeEmail:"", letterType:"offer", htmlContent:"", notes:"" });
  ...
  const handleEmployeeSelect = (emp) => {
    ...
    const tpl = templates[form.letterType];
    setForm(f => ({ ...f, employeeId: empId, employeeName: empName, employeeEmail: empEmail, htmlContent: tpl ? tpl(empName) : f.htmlContent }));
    setErrors({});
  };

  const handleEmployeeClear = () => { setSelectedEmployee(null); setForm(f => ({ ...f, employeeId:"", employeeName:"", employeeEmail:"" })); };

  const handleSend = async () => {
    const payload = { employeeId: selectedEmployee?._id || form.employeeId, employeeEmail: form.employeeEmail, employeeName: form.employeeName, letterType: form.letterType, htmlContent: form.htmlContent, notes: form.notes || "" };
    try {
      setLoading(true);
      await axios.post(`${API}/send`, payload, { headers: { "x-auth-token": token, "Content-Type": "application/json" } });
  ...
  const handleDraft = async () => {
    ...
    try {
      setLoading(true);
      await axios.post(`${API}/draft`, payload, { headers: { "x-auth-token": token, "Content-Type": "application/json" } });
      showToast("Draft saved!"); fetchHistory();
    } catch { showToast("Draft failed", false); } finally { setLoading(false); }
  };
    ...
    <div style={{ minHeight:"100vh", background:"#F9FAFB", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <main style={{ marginLeft:`${sidebarWidth}px`, transition:"margin-left 0.25s cubic-bezier(0.4,0,0.2,1)", display:"flex", flexDirection:"column", minHeight:"100vh", minWidth:0 }}>
        ...
        <div className="letter-main" style={{ padding:"28px 28px 48px" }}>
          ...
          <div className="letter-content-grid" style={{ display:"grid", gridTemplateColumns:"380px 1fr", gap:"20px", alignItems:"start" }}>
              ...
              <div style={{ padding:"20px 22px", display:"flex", flexDirection:"column", gap:"14px" }}>
                <div>
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:"7px" }}>
                    {letterTypes.map(t => {
                      ...
                      return <button key={t.value} className="type-chip" onClick={() => setField("letterType", t.value)} style={{ background:active?t.bg:"#F9FAFB", color:active?t.color:"#6B7280", borderColor:active?t.color:"transparent" }}><Icon size={12}/>{t.label}</button>;
                    })}
                  </div>
                </div>
                <div>
                ...
                <div>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"6px" }}>
                    ...
                    <button onClick={() => setPreview(form.htmlContent)} style={{ fontSize:"0.72rem", color:"#4F46E5", background:"none", border:"none", cursor:"pointer", display:"flex", alignItems:"center", gap:"4px", fontWeight:"500" }}><Eye size={12}/> Preview</button>
                  </div>
                  <textarea rows={8} placeholder="HTML content…" value={form.htmlContent} onChange={e => setField("htmlContent", e.target.value)} className="form-input" style={{ resize:"vertical", lineHeight:1.55, fontFamily:"'Courier New',monospace", fontSize:"0.78rem" }}/>
                </div>
                <div style={{ position:"relative" }}>
                      ...
                      <div key={item._id || idx} className="hist-row" style={{ padding:"16px 22px", animation:`slideIn .3s ease both ${idx*.04}s` }}>
                        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"12px" }}>
                          ...
                          <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:"5px", flexShrink:0 }}>
                            <span style={{ padding:"3px 10px", borderRadius:"20px", fontSize:"0.7rem", fontWeight:"600", background:type.bg, color:type.color, display:"inline-flex", alignItems:"center", gap:"4px" }}><Icon size={10}/>{item.letterType}</span>
