  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#F9FAFB", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .btn-secondary{display:flex;align-items:center;justify-content:center;gap:7px;padding:11px 0;border-radius:10px;font-size:0.875rem;font-weight:600;cursor:pointer;border:1.5px solid #E5E7EB;background:#fff;color:#374151;transition:all .18s;font-family:'DM Sans',sans-serif}
        .btn-secondary:hover:not(:disabled){background:#F9FAFB;transform:translateY(-1px)}
        ...
        *{box-sizing:border-box}
        ...
        @media(min-width:769px) and (max-width:1024px){
          ...
          .appr-content-grid{grid-template-columns:300px 1fr!important}
          .appr-stats-grid{grid-template-columns:repeat(3,1fr)!important}
        }
      `}</style>

      {toast && (
        <div style={{ position:"fixed", top:"20px", right:"24px", zIndex:9999, background: toast.success?"#059669":"#EF4444", color:"#fff", borderRadius:"12px", padding:"12px 18px", fontSize:"0.875rem", fontWeight:"500", boxShadow:"0 8px 24px rgba(0,0,0,.15)", display:"flex", alignItems:"center", gap:"8px", animation:"toastIn .25s ease" }}>
      ...
      <div style={{ marginLeft:`${sidebarWidth}px`, flex:1, transition:"margin-left 0.25s cubic-bezier(0.4,0,0.2,1)", display:"flex", flexDirection:"column", minHeight:"100vh", minWidth:0 }}>
        <div className="appr-topbar" style={{ height:"64px", backgroundColor:"#fff", borderBottom:"1px solid #F1F3F9", alignItems:"center", padding:"0 28px", gap:"16px", position:"sticky", top:0, zIndex:100, boxShadow:"0 1px 4px rgba(15,23,42,.04)" }}>
        ...
        <main className="appr-main" style={{ padding:"28px 28px 48px" }}>
          ...
          <div className="appr-stats-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"14px", marginBottom:"24px" }}>
            {[
              ...
              const Icon = s.icon;
              return (
                <div key={i} style={{ background:"#fff", borderRadius:"14px", padding:"18px 20px", border:"1px solid #F1F3F9", boxShadow:"0 2px 8px rgba(15,23,42,.05)", display:"flex", alignItems:"center", gap:"14px", animation:`fadeUp .4s ease both ${.1+i*.06}s` }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:"0.72rem", color:"#9CA3AF", fontWeight:"500", marginTop:"3px", textTransform:"uppercase", letterSpacing:"0.4px" }}>{s.label}</div>
                  </div>
                </div>
              );
            })}
          ...
          <div className="appr-content-grid" style={{ display:"grid", gridTemplateColumns:"360px 1fr", gap:"20px", alignItems:"start" }}>
            <div style={{ background:"#fff", borderRadius:"16px", border:"1px solid #F1F3F9", boxShadow:"0 2px 8px rgba(15,23,42,.05)", overflow:"hidden", animation:"fadeUp .4s ease both .22s" }}>
              <div style={{ padding:"20px 22px 16px", borderBottom:"1px solid #F1F3F9", background:"linear-gradient(135deg,#FAFBFF 0%,#fff 100%)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                  <div style={{ width:"38px", height:"38px", borderRadius:"10px", background:"#EEF2FF", display:"flex", alignItems:"center", justifyContent:"center", color:"#4F46E5" }}><Award size={20}/></div>
                  <div>
                    <h2 style={{ fontSize:"1rem", fontWeight:"600", color:"#111827", margin:0 }}>Send Appreciation</h2>
                    <p style={{ fontSize:"0.75rem", color:"#9CA3AF", margin:"2px 0 0" }}>Recognize a teammate today</p>
                  </div>
                </div>
              </div>
              <div style={{ padding:"20px 22px", display:"flex", flexDirection:"column", gap:"14px" }}>
                <div>
                  <label style={{ fontSize:"0.72rem", fontWeight:"600", color:"#6B7280", textTransform:"uppercase", letterSpacing:"0.5px", display:"block", marginBottom:"8px" }}>Appreciation Type</label>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", paddingTop:"4px" }}>
                  <button onClick={handleSend} disabled={loading} className="btn-primary" style={{ background:"linear-gradient(135deg,#4F46E5,#6366F1)", color:"#fff" }}>
            ...
            <div style={{ background:"#fff", borderRadius:"16px", border:"1px solid #F1F3F9", boxShadow:"0 2px 8px rgba(15,23,42,.05)", overflow:"hidden", animation:"fadeUp .4s ease both .28s" }}>
              <div style={{ padding:"18px 22px", borderBottom:"1px solid #F1F3F9", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"12px", flexWrap:"wrap" }}>
                <div>
                  <h2 style={{ fontSize:"1rem", fontWeight:"600", color:"#111827", margin:"0 0 2px" }}>Appreciation History</h2>
                ...
                <div className="appr-hist-filters" style={{ display:"flex", gap:"8px", alignItems:"center", flexWrap:"wrap" }}>
                  <div style={{ position:"relative" }}>
                    ...
                    <select value={filterType} onChange={(e)=>setFilterType(e.target.value)} className="appr-hist-select"
                      style={{ padding:"7px 12px 7px 28px", border:"1.5px solid #E5E7EB", borderRadius:"9px", fontSize:"0.78rem", color:"#374151", background:"#F9FAFB", fontFamily:"'DM Sans',sans-serif", outline:"none", cursor:"pointer", appearance:"none" }}>
                  ...
                  <div style={{ position:"relative" }}>
                    ...
                    <input className="search-inp appr-hist-search" placeholder="Search history..." value={search} onChange={(e)=>setSearch(e.target.value)}
                      style={{ padding:"7px 12px 7px 28px", border:"1.5px solid #E5E7EB", borderRadius:"9px", fontSize:"0.78rem", color:"#374151", background:"#F9FAFB", fontFamily:"'DM Sans',sans-serif", width:"190px", outline:"none" }}/>
                  </div>
                </div>
              </div>
                        ...
                        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"12px" }}>
                          <div style={{ display:"flex", gap:"12px", alignItems:"flex-start", minWidth:0 }}>
                            ...
                            <div style={{ minWidth:0 }}>
                              <h3 style={{ fontSize:"0.9rem", fontWeight:"600", color:"#111827", margin:"0 0 3px", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{item.title}</h3>
                          ...
                          <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:"6px", flexShrink:0 }}>
                            ...
                            <span style={{ padding:"2px 9px", borderRadius:"20px", fontSize:"0.68rem", fontWeight:"600", background:item.status==="sent"?"#ECFDF5":"#FFFBEB", color:item.status==="sent"?"#059669":"#D97706" }}>
                              {item.status==="sent"?"✓ Sent":"⏳ Draft"}
                            </span>
                          </div>
                        </div>
                        <p style={{ marginTop:"12px", fontSize:"0.855rem", color:"#374151", lineHeight:"1.6", borderLeft:`3px solid ${type.color}`, paddingLeft:"12px" }}>{item.message}</p>
