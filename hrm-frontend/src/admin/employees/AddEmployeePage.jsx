...
function AddEmployee() {
  ...
  const labelBase = {
    ...
    color: "#6B7280", marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.5px",
  };

  const iconBase = { position: "absolute", left: "13px", top: "50%", transform: "translateY(-50%)", color: "#9CA3AF" };

  const fields = [
    { label: "Full Name", field: "name", type: "text", icon: <UserPlus size={16} style={iconBase} />, placeholder: "John Doe" },
    { label: "Work Email", field: "email", type: "email", icon: <Mail size={16} style={iconBase} />, placeholder: "name@company.com" },
    { label: "Access Password", field: "password", type: "password", icon: <Lock size={16} style={iconBase} />, placeholder: "Min. 6 characters" },
    { label: "Contact Number", field: "phone", type: "tel", icon: <Phone size={16} style={iconBase} />, placeholder: "+91 98765 43210" },
    { label: "Joining Date", field: "joining_date", type: "date", icon: <Calendar size={16} style={iconBase} />, placeholder: "" },
  ];
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#F9FAFB", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {toast && (
        <div style={{ position: "fixed", top: "20px", right: "20px", background: toast.type === "error" ? "#EF4444" : "#059669", color: "#fff", padding: "12px 20px", borderRadius: "12px", fontWeight: "500", fontSize: "0.875rem", zIndex: 9999, boxShadow: "0 8px 24px rgba(0,0,0,0.15)", animation: "slideIn 0.2s ease both", maxWidth: "320px" }}>
          {toast.message}
        </div>
      )}
      ...
      <div style={{ marginLeft: `${sidebarWidth}px`, flex: 1, transition: "margin-left 0.25s cubic-bezier(0.4,0,0.2,1)", display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        ...
        <div className="ae-main" style={{ padding: "28px 28px 40px", flex: 1 }}>
          ...
          <div style={{ backgroundColor: "#fff", borderRadius: "16px", border: "1px solid #F1F3F9", boxShadow: "0 2px 8px rgba(15,23,42,0.05)", overflow: "hidden", animation: "fadeUp 0.4s ease both 0.15s" }}>
            <div style={{ padding: "20px 28px", borderBottom: "1px solid #F1F3F9", display: "flex", alignItems: "center", gap: "12px" }}>
              <div style={{ width: "40px", height: "40px", borderRadius: "11px", background: "#EEF2FF", display: "flex", alignItems: "center", justifyContent: "center", color: "#4F46E5" }}>
                <UserPlus size={19} />
              </div>
              <div>
                <h2 style={{ fontSize: "1rem", fontWeight: "600", color: "#111827", margin: "0 0 2px" }}>New Employee Onboarding</h2>
                <p style={{ fontSize: "0.78rem", color: "#9CA3AF", margin: 0 }}>Creates a login account + employee profile automatically</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="ae-form-grid" style={{ padding: "28px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "20px" }}>
                {fields.map(({ label, field, type, icon, placeholder }) => (
                  <div key={field}>
                    <label style={labelBase}>{label} <span style={{ color: "#EF4444" }}>*</span></label>
                    <div style={{ position: "relative" }}>
                      {icon}
                      <input className="field-input" type={type} placeholder={placeholder} value={formData[field]} onChange={handleChange(field)} required minLength={field === "password" ? 6 : undefined} style={inputBase} />
                    </div>
                  </div>
                ))}

                <div>
                  <label style={labelBase}>Department <span style={{ color: "#EF4444" }}>*</span></label>
                  <div style={{ position: "relative" }}>
                    <Briefcase size={16} style={iconBase} />
                    <select className="field-input" value={formData.department_id} onChange={handleChange("department_id")} required disabled={deptLoading}
                      style={{ ...inputBase, appearance: "none", cursor: deptLoading ? "not-allowed" : "pointer", color: formData.department_id ? "#111827" : "#9CA3AF", opacity: deptLoading ? 0.6 : 1 }}>
                    ...
                    <svg style={{ position: "absolute", right: "13px", top: "50%", transform: "translateY(-50%)", pointerEvents: "none", color: "#9CA3AF" }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="ae-footer" style={{ padding: "18px 28px", borderTop: "1px solid #F1F3F9", backgroundColor: "#FAFBFF", display: "flex", justifyContent: "flex-end", alignItems: "center", gap: "12px" }}>
                <button type="button" onClick={resetForm}
                  style={{ background: "#F1F5F9", color: "#64748b", border: "none", borderRadius: "11px", padding: "11px 20px", fontWeight: "600", fontSize: "0.875rem", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                  Reset
                </button>
