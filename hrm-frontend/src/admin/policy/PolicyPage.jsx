...
const SHARED = `
  ...
  *{box-sizing:border-box}
  .del-modal-bg{position:fixed;inset:0;background:rgba(15,23,42,.5);z-index:8000;display:flex;align-items:flex-end;justify-content:center}
  .del-modal-sheet{background:#fff;border-radius:18px 18px 0 0;width:100%;max-height:92vh;overflow-y:auto;padding:24px 20px 32px;animation:slideUp .25s ease both}
  @media(min-width:600px){
    .del-modal-bg{align-items:center;padding:16px}
    .del-modal-sheet{border-radius:16px;max-width:380px;padding:28px 32px;animation:slideIn .2s ease both}
  }
  ...
  @media(min-width:769px) and (max-width:1024px){
    ...
    .pol-content-grid{grid-template-columns:300px 1fr!important}
  }
`;
...
export default function PolicyPage() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F9FAFB", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      {deleteModal && (
        <div className="del-modal-bg" onClick={() => setDeleteModal(null)}>
          <div className="del-modal-sheet" onClick={e => e.stopPropagation()}>
            <div style={{ width:"52px", height:"52px", borderRadius:"14px", background:"#FEF2F2", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px", color:"#EF4444" }}><Trash2 size={24}/></div>
            <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.1rem", fontWeight:"700", color:"#111827", margin:"0 0 8px", textAlign:"center" }}>Delete Policy?</h3>
            <p style={{ fontSize:"0.855rem", color:"#6B7280", margin:"0 0 24px", lineHeight:1.6, textAlign:"center" }}>
              "<strong>{deleteModal.title}</strong>" will be permanently deleted along with its file.
            </p>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
              <button onClick={() => setDeleteModal(null)} style={{ padding:"10px", borderRadius:"10px", border:"1.5px solid #E5E7EB", background:"#fff", color:"#374151", fontFamily:"'DM Sans',sans-serif", fontSize:"0.875rem", fontWeight:"600", cursor:"pointer" }}>Cancel</button>
      ...
      <main style={{ marginLeft:`${sidebarWidth}px`, flex:1, transition:"margin-left 0.25s cubic-bezier(0.4,0,0.2,1)", display:"flex", flexDirection:"column", minHeight:"100vh", minWidth:0 }}>
        ...
        <div className="pol-main" style={{ padding:"28px 28px 48px" }}>
          <div style={{ marginBottom:"24px", animation:"fadeUp .4s ease both .05s" }}>
            ...
            <h1 className="pol-h1" style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.85rem", fontWeight:"700", color:"#111827", margin:0, lineHeight:1.2 }}>Policy Management</h1>
          ...
          <div className="pol-stats-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"14px", marginBottom:"24px" }}>
            {[
              ...
              const Icon = s.icon;
              return (
                <div key={i} style={{ background:"#fff", borderRadius:"14px", padding:"18px 20px", border:"1px solid #F1F3F9", boxShadow:"0 2px 8px rgba(15,23,42,.05)", display:"flex", alignItems:"center", gap:"14px", animation:`fadeUp .4s ease both ${.1+i*.06}s`, transition:"transform .15s,box-shadow .15s", cursor:"default" }}
          ...
          <div className="pol-content-grid" style={{ display:"grid", gridTemplateColumns:"340px 1fr", gap:"20px", alignItems:"start" }}>
            <div style={{ background:"#fff", borderRadius:"16px", border:"1px solid #F1F3F9", boxShadow:"0 2px 8px rgba(15,23,42,.05)", overflow:"hidden", animation:"fadeUp .4s ease both .22s" }}>
              <div style={{ padding:"20px 22px 16px", borderBottom:"1px solid #F1F3F9", background:"linear-gradient(135deg,#FAFBFF 0%,#fff 100%)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                  <div style={{ width:"38px", height:"38px", borderRadius:"10px", background:"#EEF2FF", display:"flex", alignItems:"center", justifyContent:"center", color:"#4F46E5" }}><Upload size={20}/></div>
                  <div>
              ...
              <div style={{ padding:"20px 22px", display:"flex", flexDirection:"column", gap:"14px" }}>
                ...
                <div>
                  <div className={`drop-zone${dragOver?" active":""}`} onClick={() => fileRef.current?.click()} onDragOver={e=>{e.preventDefault();setDragOver(true)}} onDragLeave={()=>setDragOver(false)} onDrop={handleDrop}>
                    {file ? (
                      ...
                      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"8px" }}>
                        <div style={{ width:"44px", height:"44px", borderRadius:"12px", background:"#EEF2FF", display:"flex", alignItems:"center", justifyContent:"center", color:"#4F46E5" }}><FolderOpen size={22}/></div>
                ...
                <button className="btn-blue" onClick={handleUpload} disabled={uploading}>
                  {uploading ? (
                    <><span style={{ width:"14px", height:"14px", border:"2px solid rgba(255,255,255,.4)", borderTopColor:"#fff", borderRadius:"50%", animation:"spin .7s linear infinite", display:"inline-block" }}/> Uploading…</>
                  ) : (
            ...
            <div style={{ background:"#fff", borderRadius:"16px", border:"1px solid #F1F3F9", boxShadow:"0 2px 8px rgba(15,23,42,.05)", overflow:"hidden", animation:"fadeUp .4s ease both .28s" }}>
              <div className="pol-table-header" style={{ padding:"18px 22px", borderBottom:"1px solid #F1F3F9", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"12px", flexWrap:"wrap" }}>
                <div>
                  ...
                  <thead>
                    <tr style={{ background:"#FAFBFF" }}>
                      {["#","Policy","File","Date","Actions"].map((h,i) => (
                        <th key={i} className={i===2?"pol-col-file":""} style={{ padding:"11px 22px", textAlign:"left", fontSize:"0.72rem", fontWeight:"600", color:"#9CA3AF", textTransform:"uppercase", letterSpacing:"0.5px", borderBottom:"1px solid #F1F3F9", whiteSpace:"nowrap", ...(i===4?{textAlign:"right"}:{}) }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      Array.from({length:5}).map((_,i) => (
                        <tr key={i}>
                          {[30,180,120,100,80].map((w,j) => (
                            <td key={j} style={{ padding:"14px 22px" }}><div style={{ height:"13px", width:`${w}px`, background:"#F3F4F6", borderRadius:"4px" }}/></td>
                          ))}
                        </tr>
                      ))
                    ) : filtered.length === 0 ? (
                          ...
                          <tr key={pol._id || i} className="pol-row" style={{ animation:`slideIn .3s ease both ${i*.03}s` }}>
                            ...
                            <td style={{ padding:"13px 22px" }}>
                              <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                                ...
                                <span style={{ fontSize:"0.875rem", fontWeight:"500", color:"#111827", whiteSpace:"nowrap" }}>{pol.title}</span>
                              </div>
                            </td>
                            ...
                            <td style={{ padding:"13px 22px" }}>
                              <div style={{ display:"flex", alignItems:"center", justifyContent:"flex-end", gap:"6px" }}>
                                <a href={`${API}/view/${pol.file}`} target="_blank" rel="noreferrer" title="View"
                                  style={{ width:"32px", height:"32px", borderRadius:"8px", border:"1.5px solid #E5E7EB", background:"#fff", display:"flex", alignItems:"center", justifyContent:"center", color:"#4F46E5", textDecoration:"none", transition:"all .15s" }}
                                  onMouseEnter={e=>{e.currentTarget.style.background="#EEF2FF";e.currentTarget.style.borderColor="#C7D2FE"}}
                                  onMouseLeave={e=>{e.currentTarget.style.background="#fff";e.currentTarget.style.borderColor="#E5E7EB"}}
                                ><Eye size={14}/></a>
                                <a href={`${API}/view/${pol.file}`} download title="Download"
                                  style={{ width:"32px", height:"32px", borderRadius:"8px", border:"1.5px solid #E5E7EB", background:"#fff", display:"flex", alignItems:"center", justifyContent:"center", color:"#059669", textDecoration:"none", transition:"all .15s" }}
                                  onMouseEnter={e=>{e.currentTarget.style.background="#ECFDF5";e.currentTarget.style.borderColor="#A7F3D0"}}
                                  onMouseLeave={e=>{e.currentTarget.style.background="#fff";e.currentTarget.style.borderColor="#E5E7EB"}}
                                ><Download size={14}/></a>
                                <button className="del-btn" onClick={() => setDeleteModal(pol)} title="Delete"
                                  style={{ width:"32px", height:"32px", borderRadius:"8px", border:"1.5px solid #E5E7EB", background:"#fff", display:"flex", alignItems:"center", justifyContent:"center", color:"#9CA3AF", cursor:"pointer", transition:"all .15s" }}>
                                  <Trash2 size={14}/>
                                </button>
                              </div>
                            </td>
